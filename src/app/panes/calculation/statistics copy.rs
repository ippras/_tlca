use crate::{
    markdown::{
        BRAY_CURTIS_DISSIMILARITY, CHEBYSHEV_DISTANCE, COSINE_SIMILARITY, EUCLIDEAN_DISTANCE,
        JACCARD_DISTANCE, JENSEN_SHANNON_DISTANCE, MANHATTAN_DISTANCE,
        PEARSON_CORRELATION_COEFFICIENT, SPEARMAN_CORRELATION_COEFFICIENT,
    },
    utils::AnyValueExt as _,
};
use egui::{Color32, Grid, Label, RichText, Ui, Widget};
...
impl Statistics<'_> {
    ...
    pub(crate) fn show(&mut self, ui: &mut Ui) -> PolarsResult<()> {
        ...
        Grid::new(ui.next_auto_id()).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.menu_button(INFO, |ui| ui.markdown(SPEARMAN_CORRELATION_COEFFICIENT));
